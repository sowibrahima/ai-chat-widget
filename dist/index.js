...
Object.defineProperty(exports, "CourseGenerationButton", {
  enumerable: true,
  get: function () {
    return _CourseGenerationButton.default;
  }
});
Object.defineProperty(exports, "CourseGenerationModal", {
  enumerable: true,
  get: function () {
    return _CourseGenerationModal.default;
  }
});
...
exports.openCourseGenerationModal = openCourseGenerationModal;
...
var _CourseGenerationButton = _interopRequireDefault(require("./CourseGenerationButton"));
...
function openCourseGenerationModal() {
  let options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const {
  ...
  if (window.React && window.ReactDOM && _CourseGenerationModal.default) {
    const modalProps = {
      isOpen: true,
      courseId: courseId,
      onClose: handleClose,
    ...
    window.ReactDOM.render(window.React.createElement(_CourseGenerationModal.default, modalProps), modalContainer);
  } else {
...
function handleSimpleFormSubmit(courseId, onSuccess, onError) {
  ...
  }).then(response => response.json()).then(jobResult => {
...
function startSimplePolling(jobId, progressBar, statusText, onSuccess, onError) {
  const pollInterval = setInterval(() => {
    ...
    }).then(response => response.json()).then(jobData => {
...
if (typeof window !== 'undefined') {
  ...
  window.WSAIAssistant.CourseGenerationModal = _CourseGenerationModal.default;
  window.WSAIAssistant.CourseGenerationButton = _CourseGenerationButton.default;
